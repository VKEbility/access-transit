...
export default function RouteCarousel({ routes, handleTimerEnd }) {
  const renderSlide = (route, itinerary, direction) => (
    <Carousel.Slide key={direction} className={classes}>
      <TransitRouteCard
        route={{ ...route, itineraries: [itinerary] }}
  ...
  return (
    <Box sx={{ display: 'flex', justifyContent: 'center', margin: '20px 0' }}>
      {routes.map((route) => {
        const uptownItinerary = route.itineraries.find(it => it.directionId === 0) || {};
        const downtownItinerary = route.itineraries.find(it => it.directionId === 1) || {};

        return (
          <Box key={route.globalRtId} sx={{ minHeight: '200px', width: '100%', padding: '0 10px' }}
