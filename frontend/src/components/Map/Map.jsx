...
export default function MapContainerComponent({ setCoords, setMapReady }) {
	const [position, setPosition] = useState([40.7128, -74.006]); //state to hold the default marker position; init to lower manhattan- same as in landing
	const [loading, setLoading] = useState(true);

	useEffect(() => {
		//effect to get the user's curr location when the component mounts/starts up
		const getLocation = () => {
	...
	useEffect(() => {
		//effect to update parent coords when position state of the marker changes
